...
const Profile = () => {
  ...
  const displayedPosts = activeTab === 'post' ? userProfile?.posts : userProfile?.bookmarks;
          ...
          <div className="flex items-center gap-4">
            ...
            {isCurrentUser ? (
              <div className="flex gap-2">
                <Button variant="secondary" className="text-sm h-7">Edit Profile</Button>
                <Button variant="secondary" className="text-sm h-7">View Archive</Button>
              </div>
            ) : (
              <div className="flex gap-2 items-center">
                <Button
                  onClick={() => setIsFollowing(!isFollowing)}
                  className={`text-sm h-7 ${isFollowing ? 'bg-gray-200 hover:bg-gray-300 text-black' : 'bg-insta-primary hover:bg-insta-hoverPrimary text-white'}`}
                >
                  {isFollowing ? 'Unfollow' : 'Follow'}
                </Button>
                <Button className="text-sm h-7 bg-gray-200 text-black hover:bg-gray-300">Message</Button>
              </div>
            )}
          </div>
